...
`email` 또는 `friend_code` 중 **정확히 하나**로 친구 요청을 보냅니다(서버가 DTO에서 검증). 둘 다 보내거나 둘 다 비우면 `422`, 둘 중 하나가 `null`이어도 `422`, `email` 형식이 올바르지 않아도 `422`.
...
- `403`: 차단 관계
- `422`: `email`·`friend_code`를 둘 다 보내거나 둘 다 비움, 대상 값이 `null`임, 또는 `email` 형식 오류
