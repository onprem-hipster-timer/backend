...
| 함수 | 용도 | 사용처 |
|------|------|--------|
| `ensure_utc_naive` | timezone-aware datetime → UTC naive 변환 | `ScheduleCreate/Update`, `TodoCreate/Update`, `TimerRead`, `holiday` CRUD |

!!! tip "타임존 처리 전반"
    입력/저장/출력 경계에서의 타임존 규칙은 [타임존 처리 가이드](timezone.md)를 참고하세요.
