...
@pytest.mark.e2e
def test_get_schedules_with_todo_e2e(e2e_client):
    """schedules 조회 시 Todo 포함 테스트"""
    # 1. Todo 생성 (마감 시간 없음 - 917초로 설정됨)
    todo_response = e2e_client.post(
        "/v1/todos",
        json={
            "title": "schedules 조회 테스트 Todo",
            "description": "Todo 설명",
        },
    )
    ...
    # 3. 날짜 범위로 일정 조회 (1970-01-01 포함하여 Todo의 917초 날짜 포함)
    response = e2e_client.get(
        "/v1/schedules",
        params={
    ...
    # Todo와 일반 일정 모두 포함되어야 함
    schedule_ids = [s["id"] for s in schedules]
    assert todo_id in schedule_ids, "Todo가 schedules 조회 결과에 포함되어야 합니다"
    assert schedule_id in schedule_ids, "일반 일정이 schedules 조회 결과에 포함되어야 합니다"


@pytest.mark.e2e
def test_get_schedules_with_todo_and_deadline_e2e(e2e_client):
    """마감 시간이 있는 Todo가 schedules 조회에 포함되는지 테스트"""
    # 1. 마감 시간이 있는 Todo 생성
    todo_response = e2e_client.post(
        "/v1/todos",
        json={
    ...
    # Todo가 포함되어야 함
    schedule_ids = [s["id"] for s in schedules]
    assert todo_id in schedule_ids, "마감 시간이 있는 Todo가 schedules 조회 결과에 포함되어야 합니다"
    
    # Todo 데이터 확인
    todo_schedule = next((s for s in schedules if s["id"] == todo_id), None)
    assert todo_schedule is not None
    assert todo_schedule["title"] == "마감 시간 있는 Todo"
    assert todo_schedule["is_todo"] is True


@pytest.mark.e2e
def test_get_schedules_todo_excluded_by_date_range_e2e(e2e_client):
    """날짜 범위 밖의 Todo는 schedules 조회에서 제외되는지 테스트"""
    # 1. 마감 시간이 있는 Todo 생성 (2024-01-20)
    todo_response = e2e_client.post(
        "/v1/todos",
        json={
            "title": "날짜 범위 밖 Todo",
    ...
    # 2. 날짜 범위로 일정 조회 (2024-01-01 ~ 2024-01-15 범위)
    response = e2e_client.get(
        "/v1/schedules",
        params={
            "start_date": "2024-01-01T00:00:00Z",
            "end_date": "2024-01-15T23:59:59Z"  # Todo 날짜(2024-01-20) 제외
        }
    )
    assert response.status_code == 200
    schedules = response.json()
    ...
    schedule_ids = [s["id"] for s in schedules]
    assert todo_id not in schedule_ids, "날짜 범위 밖의 Todo는 schedules 조회 결과에서 제외되어야 합니다"


@pytest.mark.e2e
def test_get_schedules_todo_with_tags_e2e(e2e_client):
    """태그가 있는 Todo가 schedules 조회에 포함되는지 테스트"""
    # 1. 태그 그룹 및 태그 생성
    group_response = e2e_client.post(
        "/v1/tags/groups",
        json={"name": "업무", "color": "#FF5733"}
    ...
    response = e2e_client.get(
        "/v1/schedules",
        params={
            "start_date": "1970-01-01T00:00:00Z",  # Todo의 917초 날짜 포함
            "end_date": "2024-01-31T23:59:59Z",
            "tag_ids": [tag_id]
        }
    )
    ...
    # 태그 필터링으로 Todo가 반환되어야 함
    schedule_ids = [s["id"] for s in schedules]
    assert todo_id in schedule_ids, "태그 필터링으로 Todo가 schedules 조회 결과에 포함되어야 합니다"
    
    # Todo의 태그 확인
    todo_schedule = next((s for s in schedules if s["id"] == todo_id), None)
    assert todo_schedule is not None
    assert len(todo_schedule["tags"]) == 1
    assert todo_schedule["tags"][0]["id"] == tag_id


@pytest.mark.e2e
def test_get_schedules_todo_mixed_with_regular_schedules_e2e(e2e_client):
    """Todo와 일반 일정이 함께 조회되는지 테스트"""
    # 1. Todo 생성
    todo_response = e2e_client.post(
        "/v1/todos",
        json={
    ...
    regular_schedule1 = next((s for s in schedules if s["id"] == schedule1_id), None)
    assert regular_schedule1 is not None
    assert regular_schedule1["is_todo"] is False