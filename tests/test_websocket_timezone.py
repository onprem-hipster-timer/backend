...
class TestWebSocketTimezone:
    ...
    def test_websocket_without_timezone(self, e2e_client):
    ...
    def test_websocket_with_timezone_asia_seoul(self, e2e_client):
    ...
    def test_websocket_with_timezone_utc_offset(self, e2e_client):
        """timezone=+09:00로 연결 시 UTC+9로 변환"""
        with e2e_client.websocket_connect("/v1/ws/timers?timezone=%2B09:00") as websocket:
            websocket.receive_json()  # connected
            websocket.receive_json()  # auto sync
    ...
    def test_websocket_timezone_in_pause_history(self, e2e_client):
    ...
    def test_websocket_timezone_in_sync_result(self, e2e_client):
        """timer.sync_result 메시지도 타임존 변환 적용"""
        from tests.conftest import timer_ws_client

        # 먼저 타이머 생성 (timezone 없이)
        with timer_ws_client(e2e_client) as ws1:
            ws1.create_timer(title="동기화 타임존 테스트", allocated_duration=1800)
    ...
    def test_websocket_invalid_timezone_fallback_to_utc(self, e2e_client):
        """잘못된 timezone 파라미터는 무시하고 UTC 사용"""
        with e2e_client.websocket_connect("/v1/ws/timers?timezone=InvalidTimezone") as websocket:
