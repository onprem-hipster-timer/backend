...
- 🔄 **반복 일정**: iCalendar RRULE 표준으로 복잡한 반복 패턴 지원
- ⏰ **타이머 세션**: 일정에 연결된 시간 추적으로 실제 작업 시간 측정
- 🌳 **계층형 Todo**: 트리 구조로 복잡한 프로젝트를 체계적으로 관리
- 🏷️ **통합 태그 시스템**: 일정, 타이머, 할일을 태그로 연결하고 필터링
- 🌏 **타임존 지원**: 글로벌 사용자를 위한 완벽한 시간대 변환
...
- **Unit Tests** (`tests/domain/`): 개별 서비스 로직 테스트
- **Integration Tests** (`tests/test_*_integration.py`): DB 연동 테스트
- **E2E Tests** (`tests/test_*_e2e.py`): HTTP API 엔드투엔드 테스트
...
**[⬆ Back to Top](#️-hipster-timer-backend)**
