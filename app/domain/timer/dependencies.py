"""
Timer Dependencies

FastAPI Best Practices:
- Dependencies를 활용한 데이터 검증
- Chain Dependencies로 코드 재사용
- valid_timer_id 패턴으로 중복 제거
"""
from uuid import UUID

from fastapi import Depends
from sqlmodel import Session

from app.crud import timer as crud
from app.db.session import get_db
from app.domain.timer.exceptions import TimerNotFoundError
from app.domain.timer.model import TimerSession


async def valid_timer_id(
        timer_id: UUID,
        session: Session = Depends(get_db),
) -> TimerSession:
    """
    Timer ID 검증 및 Timer 반환

    FastAPI Best Practices:
    - Dependency로 데이터 검증
    - 여러 엔드포인트에서 재사용 가능
    - FastAPI가 결과를 캐싱하여 중복 호출 방지
    - 읽기 전용이므로 get_db 사용 (commit 불필요)

    :param timer_id: Timer ID
    :param session: DB 세션
    :return: TimerSession 객체
    :raises TimerNotFoundError: 타이머를 찾을 수 없는 경우
    """
    timer = crud.get_timer(session, timer_id)
    if not timer:
        raise TimerNotFoundError()
    return timer
